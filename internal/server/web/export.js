...
<div id="flt-nav" hidden>
  <span id="flt-count"></span>
  <button onclick="fltPrev()" title="Previous match (Shift+Enter)">&#8593;</button>
  <button onclick="fltNext()" title="Next match (Enter)">&#8595;</button>
</div>
<script>
${exportJs()}
</script>
...
mark.flt-mark.flt-current{background:#ff9f1c}
#flt-nav{position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:var(--surface);border:1px solid var(--border);color:var(--text);border-radius:7px;padding:5px 8px;display:flex;align-items:center;gap:4px;font-size:12px;box-shadow:0 2px 10px rgba(0,0,0,.25);z-index:50}
#flt-nav[hidden]{display:none}
#flt-count{min-width:44px;text-align:center;opacity:.8}
#flt-nav button{background:none;border:none;color:var(--text);cursor:pointer;padding:3px 7px;border-radius:5px;display:flex;align-items:center}
#flt-nav button:hover{background:rgba(128,128,128,.25)}
...
var fltMatches=[],fltIdx=0;

// Removes all filter-match <mark> wrappers, restoring the original text
// nodes, so the next filter pass starts from a clean slate.
function clearHighlights(){
  document.querySelectorAll('#outline mark.flt-mark').forEach(function(m){
...
// Wraps every case-insensitive occurrence of q in el's text (skipping
// script/style/existing marks) in a <mark>, walking only text nodes so
// existing formatting (bold, links, tables) is left completely intact.
...
function updateFltCurrent(){
  fltMatches.forEach(function(m,i){m.classList.toggle('flt-current',i===fltIdx);});
  var countEl=document.getElementById('flt-count');
  if(countEl)countEl.textContent=(fltMatches.length?(fltIdx+1):0)+' / '+fltMatches.length;
  if(fltMatches[fltIdx])fltMatches[fltIdx].scrollIntoView({block:'center',behavior:'smooth'});
}

function updateFindNav(){
  fltMatches=[].slice.call(document.querySelectorAll('#outline mark.flt-mark'));
  var nav=document.getElementById('flt-nav');
  if(!nav)return;
  if(fltMatches.length===0){nav.hidden=true;fltIdx=0;return;}
  nav.hidden=false;
  fltIdx=0;
  updateFltCurrent();
}
...
function fltNext(){
  if(!fltMatches.length)return;
  fltIdx=(fltIdx+1)%fltMatches.length;
  updateFltCurrent();
}

function fltPrev(){
  if(!fltMatches.length)return;
  fltIdx=(fltIdx-1+fltMatches.length)%fltMatches.length;
  updateFltCurrent();
}
...
function applyFilters(){
  ...
  clearHighlights();
  if(q){
    document.querySelectorAll('#outline .ns:not([hidden])').forEach(function(el){
      highlightIn(el,q);
    });
  }
  updateFindNav();
